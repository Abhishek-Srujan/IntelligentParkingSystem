...
public class EventServlet extends EventSourceServlet {
    ...
    private final ClientRegistryListener clientRegistryListener = new ClientRegistryListener() {
        ...
        private void notifyServer(String client) throws IOException {
			// TODO Auto-generated method stub
        	int clientStart = client.toString().indexOf("endpoint=")+9;
            int clientEnd=client.toString().indexOf(",",clientStart);
            String clientID = client.toString().substring(clientStart, clientEnd);
            String id="null";
            String value="null";
            sendGET("http://192.168.0.191:8081/IoT/update?id="+id+"&value="+value+"&clientID="+clientID);
            
		}

		private static final String USER_AGENT = "Mozilla/5.0";
    	private void sendGET(String command) throws IOException {
    ...
    private final ObservationRegistryListener observationRegistryListener = new ObservationRegistryListener() {
        ...
        public void newValue(Observation observation, LwM2mNode value) {
        	System.out.println(observation.toString() + value.toString() );
        	
        	
            Client client = server.getClientRegistry().findByRegistrationId(observation.getRegistrationId());
            
            try {
				notifyServer(value.toString(),client.toString());
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
        ...
        private void notifyServer(String message, String client) throws IOException {
            ...
            int newval = message.indexOf(",") +1;
            String value = message.substring(message.indexOf("=", newval)+1, message.indexOf(",",newval));
